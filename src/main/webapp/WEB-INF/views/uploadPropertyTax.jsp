		...
		<div class="card shadow-lg">
			...
			<div class="card-body">
				...
				<form method="post" enctype="multipart/form-data"
					action="${pageContext.request.contextPath}/uploadPropertyTaxCsv">
					...
					<div class="row g-3">
						...
						<div class="col-md-3">
							<a
								href="${pageContext.request.contextPath}/downloadTaxTemplateForPropertyTax"
								class="btn btn-outline-primary w-100"> ⬇ <spring:message
									code="propertyType.download_template" />
							</a>
						</div>
				...
				<div id="resultSection">
					...
					<c:if test="${not empty totalCount}">
						<div class="alert alert-info mt-3 text-center">Total:
							${totalCount} | Success: ${successCount} | Failed: ${failCount}</div>
					</c:if>
					...
					<c:if test="${not empty successList}">
						<div class="card mt-3">
							<div class="card-header bg-success text-white">Uploaded
								Records Preview</div>
							...
							<table class="table text-center">
								...
								<tbody>
									<c:forEach var="t" items="${successList}">
										<tr>
											<td>${t.property.propertyNo}</td>
											<td>${t.financialYear}</td>
											<td>${t.totalTax}</td>
											<td>${t.balanceAmount}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
